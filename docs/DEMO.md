...
6. Rename an Agent inline. Open its **•••** menu to dismiss it, undo, then merge two suggestions. The menu also supports Enter, arrow keys, and Escape.
7. Check the Agent/file/source counts, choose **Review … Agents**, inspect the destination and exact file manifest, then create the folders. For a disposable walkthrough, use **Change destination** to choose an empty test folder.
